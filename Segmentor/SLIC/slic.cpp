...
// CUSTOM FUNCTIONS
void Slic::print_clusters(IplImage *image){
  for (int i = 0; i < image->width; i++) {
    for (int j = 0; j < image->height; j++) {
...
void Slic::label_pixels(IplImage *image, cv::Mat res){
  ...
  for (int i = 0; i < image->width; i++) {
	...
	if (x >= 0 && x < image->width && y >= 0 && y < image->height) {
      ...
      /* Determine this point is a border border point */
      if (nr_p >= 1) {
	is_border = true;
      }
      if(spixels.find(index) == spixels.end()){
	spixels[index].create(0,3,CV_32S);
      }
      cv::Mat pixels(1,3,CV_32S);
      pixels.at<int>(0, 0) = i;
      pixels.at<int>(0, 1) = j;
      pixels.at<int>(0, 2) = is_border;
      //std::cout << "pixels cols:" << pixels.cols;
      spixels[index].push_back(pixels);
    }
  }
  /*
  for(map<int,cv::Mat>::iterator it = spixels.begin(); it!= spixels.end(); it++){
    ...
    for(int i = 0; i < (*it).second.rows; i++){
      std::cout << "(" << ((*it).second).at<int>(i, 0)
		<< "," << ((*it).second).at<int>(i, 1)
		<< "-" << ((*it).second).at<int>(i, 2)
		<< ")";
      }
      std::cout << std::endl;
  }
  */
  ...
  int count = 0;
  for(map<int,cv::Mat>::iterator it = spixels.begin(); it!= spixels.end(); it++){
    count += (*it).second.rows;
    //std::cout << "cols: " << (*it).second.cols;
    for(int i = 0; i < (*it).second.rows; i++){ 
